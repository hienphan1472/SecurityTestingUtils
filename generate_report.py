...
file_handle_write.write(
    "<head><link rel=\"StyleSheet\" href=\"style.css\" type=\"text/css\" media=\"screen\" /><title>SQLMap HTML Report</title></head>")
file_handle_write.write("<body><table id=\"myStyle\">")
file_handle_write.write(
    "<thead><tr><th scope=\"col\">Test datetime</th><th scope=\"col\">Test description</th></tr></thead>")
...
for line in file_handle_read:
    if (line.strip().startswith("[")) and (line.find("[*]") == -1):
        ...
        if line.lower().find("all tested parameters do not appear to be injectable") > -1:
...
# Write global audit status line
